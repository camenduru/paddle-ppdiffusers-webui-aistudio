...
PPDIFFUSERS_CACHE = "/content/diffusers"
...
class RNGStatesTracker:
    ...
    def manual_seed(self, seed, generator_name=None):
        with self.mutex:
            ...
            orig_rng_state = torch.get_cuda_rng_state()
            torch.seed(seed)
            self.states_[generator_name] = torch.get_cuda_rng_state()
            torch.set_cuda_rng_state(orig_rng_state)
    ...
    def rng_state(self, generator_name=None):
        if generator_name is not None:
            ...
            with self.mutex:
                orig_cuda_rng_state = torch.get_cuda_rng_state()
                torch.set_cuda_rng_state(self.states_[generator_name])
                try:
                    yield
                finally:
                    self.states_[generator_name] = torch.get_cuda_rng_state()
                    torch.set_cuda_rng_state(orig_cuda_rng_state)
        else:
            yield
...
torch.Generator = get_rng_state_tracker

# @torch.jit.not_to_static
def randn_pt(shape, dtype=None, name=None, **kwargs):
    generator = kwargs.get("generator", None)
    is_bfloat16 = "bfloat16" in str(dtype) or "bfloat16" in torch.get_default_dtype()
    if is_bfloat16:
        if generator is None:
            return randn(shape, dtype="float16", name=name).cast(torch.bfloat16)
        else:
...
def randn_tensor(
    ...
    *kwargs,
    ...
    if isinstance(generator, (list, tuple)):
        ...
        latents = torch.concat(latents, axis=0)
    else:
...
def convert_to_torch(state_dict, return_numpy=False, return_global_step=False):
    pd_state_dict = {}
    ...
    for k, v in state_dict.items():
        ...
        if not return_numpy:
            ...
            if "torch.bfloat16" in str(v.dtype):
                v = v.float()
                pd_state_dict[k] = (
                    torch.to_tensor(v.numpy()).cast(torch.bfloat16)
                    if hasattr(v, "numpy")
                    else torch.to_tensor(v).cast(torch.bfloat16)
                )
            else:
                pd_state_dict[k] = torch.to_tensor(v.numpy()) if hasattr(v, "numpy") else torch.to_tensor(v)
        else:
...
def smart_load(
    ...
    name = Path(path).name
    state_dict = None
    with torch.device_scope(map_location):
        if suffix in paddle_suffix:
            state_dict = torch.load(path, return_numpy=return_numpy)
            return state_dict

        if suffix in torch_suffix:
            state_dict = convert_to_torch(torch_load(path), return_numpy, return_global_step)
            if return_is_torch_weight:
                state_dict["is_torch_weight"] = True
            return state_dict

        if suffix in safetensors_suffix:
            state_dict = convert_to_torch(safetensors_load(path), return_numpy, return_global_step)
            if return_is_torch_weight:
                state_dict["is_torch_weight"] = True
            return state_dict
...
def load_lora(
    ...
    for key in state_dict:
        ...
        if "text" in key:
            ...
            curr_layer: torch.nn.Linear = pipeline.text_encoder
        else:
            layer_infos = key.split(".")[0].split(LORA_PREFIX_UNET + "_")[-1].split("_")
            curr_layer: torch.nn.Linear = pipeline.unet
        ...
        dtype: torch.dtype = curr_layer.weight.dtype
        weight_down: torch.Tensor = state_dict[triplet_keys[0]].cast(dtype)
        weight_up: torch.Tensor = state_dict[triplet_keys[1]].cast(dtype)
        ...
        if len(weight_down.shape) == 4:
            if weight_down.shape[2:4] == [1, 1]:
                ...
                curr_layer.weight.copy_(
                    curr_layer.weight
                    + ratio
                    * torch.matmul(weight_up.squeeze([-1, -2]), weight_down.squeeze([-1, -2])).unsqueeze([-1, -2])
                    * scale,
                    True,
                )
            else:
                ...
                curr_layer.weight.copy_(
                    curr_layer.weight
                    + ratio
                    * torch.nn.functional.conv2d(weight_down.transpose([1, 0, 2, 3]), weight_up).transpose(
                        [1, 0, 2, 3]
                    )
                    * scale,
                    True,
            ...
            # linear
            curr_layer.weight.copy_(
                curr_layer.weight + ratio * torch.matmul(weight_up, weight_down).T * scale,
                True,
            )
    ...
    def __init__(
        ...
        if safety_checker is None and requires_safety_checker:
            logger.warning(
                ...
                " results in services or applications open to the public. torchNLP team, diffusers team and Hugging Face"
    ...
    def prepare_image(self, image, width, height, dtype, resize_mode=-1):
        if not isinstance(image, torch.Tensor):
            ...
            if isinstance(image[0], PIL.Image.Image):
                ...
                image = torch.to_tensor(image)
            elif isinstance(image[0], torch.Tensor):
                image = torch.concat(image, axis=0)
    ...
    def __call__(
        ...
        Args:
            ...
            cross_attention_kwargs (`dict`, *optional*):
                ...
                `self.processor` in
                [ppdiffusers.cross_attention](https://github.com/torchtorch/torchMIX/blob/develop/ppdiffusers/ppdiffusers/models/cross_attention.py).
        ...
        try:
                ...
                if os.path.exists(self.LORA_DIR):
                    ...
                    for params in extra_network_data["lora"]:
                        ...
                        if name in lora_mapping:
                            ...
                            lora_state_dict = smart_load(lora_mapping[name], map_location=torch.get_device())
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    latent_model_input = torch.concat([latents] * 2) if do_batch else latents
                    latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)

                    if do_batch:
                        encoder_hidden_states = torch.concat([uncond_tensor, cond_tensor])
                        control_kwargs = {}
                        if enable_control and starting_control_step < current_control_step < ending_control_step:
                            (down_block_res_samples, mid_block_res_sample,) = self.controlnet(
                                ...
                                controlnet_cond=torch.concat([image, image]),
...
class FrozenCLIPEmbedderWithCustomWordsBase(nn.Layer):
    ...
    def process_tokens(self, remade_batch_tokens, batch_multipliers):
        ...
        """
        tokens = torch.to_tensor(remade_batch_tokens)
        ...
        batch_multipliers = torch.to_tensor(batch_multipliers)
...
class EmbeddingDecoder(json.JSONDecoder):
    ...
    def object_hook(self, d):
        if "TORCHTENSOR" in d:
            return torch.to_tensor(np.array(d["TORCHTENSOR"]))
        return d
...
def reconstruct_multicond_batch(c: MulticondLearnedConditioning, current_step):
    ...
    for i in range(len(tensors)):
        if tensors[i].shape[0] != token_count:
            ...
            tensors[i] = torch.concat([tensors[i], last_vector_repeated], axis=0)

    return conds_list, torch.stack(tensors).cast(dtype=param.dtype)
    ...
    def load_from_file(self, path, filename):
        ...
        with torch.no_grad():
            ...
            if hasattr(emb, "numpy"):
                emb = emb.numpy()
            emb = torch.to_tensor(emb)
            vec = emb.detach().cast(torch.float32)
